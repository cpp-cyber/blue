#!/bin/sh
# @d_tranman/Nigel Gerald/Nigerald's inventory, but with added features - adam
        ...
        MYSQL_OUT="$(
            {
                ...
                get_mysql_config_paths | while read -r p; do
                    ...
                    echo "--- $p"
                    if [ -f "$p" ]; then
                        grep -HiE '^\s*(bind-address|port|socket|user|skip-networking|skip-name-resolve)' "$p" 2>/dev/null | head -n 20
                    elif [ -d "$p" ]; then
                        for f in "$p"/*; do
                            [ -f "$f" ] || continue
                            grep -HiE '^\s*(bind-address|port|socket|user|skip-networking|skip-name-resolve)' "$f" 2>/dev/null | head -n 10
                        done
                    fi
                done
                echo
...
bootstrap_tmux() {
    ...
    tmux send-keys -t "${TMUX_SESSION_NAME}:0.2" \
        "clear; ( grep -vE '(false|nologin|sync)$' /etc/passwd 2>/dev/null | grep -E '/.*sh$' | awk -F: '{print \$1\":\"\$6\":\"\$7}' ) | tee \"$ENUM_ROOT/local_users.txt\"; echo; exec bash || exec sh" C-m
