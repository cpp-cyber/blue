...
echo ""
( netstat -tlpn | tail -n +3 | awk '{print $1 " " $4 " " $6 " " $7}'| column -t || ss -blunt -p | tail -n +2 | awk '{print $1 " " $5 " " $7}' | column -t )
echo ""
...
checkService()
{
    ...
    if [ -n "$serviceAlias" ]; then
        ...
        if echo "$serviceList" | grep -qi "$serviceAlias" ; then
            if [ "$( netstat -tulpn | grep -i \"$serviceAlias\" )" ] || [ "$( ss -blunt -p | grep -i "$serviceAlias" )" ]; then
        ...
        echo -e "\n${BLUE}[+] $serviceToCheckExists is on this machine${NC}\n"
        if [ "$( netstat -tulpn | grep -i \"$serviceToCheckExists\" )" ] || [ "$( ss -blunt -p | grep -i "$serviceToCheckExists" )" ]; then
